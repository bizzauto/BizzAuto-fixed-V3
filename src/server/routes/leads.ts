    ...
    for (const contact of contacts) {
      try {
        if (channel === 'whatsapp' && contact.phone) {
          await WhatsAppSendRouter.sendTextMessage(businessId, contact.phone, message, { messageId: contact.id });
          sent++;
...
router.delete('/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    await prisma.contact.delete({ where: { id: qs(id) } });
  ...
  try {
    ...
    if (phone) {
      try {
        ...
        await WhatsAppSendRouter.sendTextMessage(businessId, phone, msg, { messageId: contact.id });
      } catch (e: any) {
